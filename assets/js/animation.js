$(function(){
    ...
    if (jQuery.browser.mobile === false && !isAndroid)
    {
        ...
        /*  Schedule.php
        /*---------------------------------------*/
        $($('.section-title')[0]).css('opacity', 0).one('inview', function(isInView){
            if (isInView) {$(this).addClass('animated flipInX delayp1').css('opacity', 1);}
        });
        $('table').css('opacity', 0).one('inview', function(isInView){
        ...
        $(window).load(function() {
          $('.inview-animation-target').on('inview', function(event, isInView) {
            var $dontBeShy = $(this)
            setTimeout(function () {
              if (isInView) {
                $('.inview-animation-fold-unfold').addClass('animation-fold-unfold')
              }
            }, 500)
            setTimeout(function () {
              if (isInView) {
                $dontBeShy.removeClass('invisible').addClass('animation-roadrunner')
                // remove block space after animation
                setTimeout(function () {
                  $dontBeShy.animate({ height: '0px' })
                }, 2000)
              }
            }, 2500)
          })
